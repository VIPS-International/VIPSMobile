...
Ext.define('VIPSMobile.view.dashboard.charts.SQLWebReportNode', {
    extend: 'Ext.Container',
    alias: ['SQLWebReportNode'],
    ...
    config: {
        ...
        items: {
            ...
            handler: function () {
                if (this.parent.getNode() && this.parent.getController()) {
                    this.parent.generate(this.parent.getController(), this.parent.getNode());
                }
            },
        }
    },
    ...
    setup: function (controller, node) {
        ...
        this.setNode(node);
        this.items.items[0].setText(node.raw.Description);
    },
    ...
    generate: function (controller, node) {
        ...
        VIPSMobile.Main.setMask('Dashboard', 'Preparing...');
    ...
    SetupReportDetail: function (response) {
        ...
        popup.setup(strHtml);
        popup.items.items[0].setTitle(this.getNode().raw.Description);
        Ext.Viewport.add(popup);
        
        Ext.iterate(popup.element.dom.querySelectorAll('img'), function (i) {
            i.src = i.src + '&_dc=' + Date.now().toString();
        });

        Ext.iterate(popup.element.dom.querySelectorAll('a'), function (aHref) {
        ...
        VIPSMobile.Main.setMask('Dashboard', false);
